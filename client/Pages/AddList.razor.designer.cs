...
{
    public partial class AddListComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Sde5.Models.Sde.List args)
        {
            try
            {
                var sdeCreateListResult = await Sde.CreateList(list:list);
                DialogService.Close(list);
            }
            catch (System.Exception sdeCreateListException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new List!" });
            }
        }
