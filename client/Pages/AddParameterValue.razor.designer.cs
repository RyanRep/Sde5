...
{
    public partial class AddParameterValueComponent : ComponentBase
    {
        ...
        {
            parametervalue = new Sde5.Models.Sde.ParameterValue(){};
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Sde5.Models.Sde.ParameterValue args)
        {
            try
            {
                var sdeCreateParameterValueResult = await Sde.CreateParameterValue(parameterValue:parametervalue);
                DialogService.Close(parametervalue);
            }
            catch (System.Exception sdeCreateParameterValueException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new ParameterValue!" });
            }
        }
