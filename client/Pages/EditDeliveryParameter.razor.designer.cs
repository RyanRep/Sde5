...
{
    public partial class EditDeliveryParameterComponent : ComponentBase
    {
        ...
        [Parameter]
        public dynamic DeliveryParameterId { get; set; }
        ...
        {
            var sdeGetDeliveryParameterByDeliveryParameterIdResult = await Sde.GetDeliveryParameterByDeliveryParameterId(deliveryParameterId:DeliveryParameterId);
            deliveryparameter = sdeGetDeliveryParameterByDeliveryParameterIdResult;
        ...
        protected async System.Threading.Tasks.Task Form0Submit(Sde5.Models.Sde.DeliveryParameter args)
        {
            try
            {
                var sdeUpdateDeliveryParameterResult = await Sde.UpdateDeliveryParameter(deliveryParameterId:DeliveryParameterId, deliveryParameter:deliveryparameter);
                DialogService.Close(deliveryparameter);
            }
            catch (System.Exception sdeUpdateDeliveryParameterException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update DeliveryParameter" });
            }
        }
